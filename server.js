...
async function borrarDatosTienda(storeId) {
  await supabase.from('stock_suscripciones').delete().eq('store_id', storeId);
  await supabase.from('stock_productos').delete().eq('store_id', storeId);
  await supabase.from('aviso_eventos').delete().eq('store_id', storeId);
  await supabase.from('stock_tiendas').delete().eq('store_id', storeId);
}
    ...
    if (accesoActivo && diasRestantes !== null) {
      ...
      bannerTrial = `
        <div class="trial-banner ${diasRestantes <= 2 ? 'trial-banner--urgente' : ''}">
          ...
          <div class="trial-bar-track"><div class="trial-bar-fill" style="width:${pctTrial}%"></div></div>
        </div>`;
...
<style>
  ...
  *{ box-sizing:border-box; margin:0; padding:0; }
  ...
  .status-hero{
    ...
    box-shadow:var(--sh-sm); padding:22px 24px; margin-bottom:8px; animation:fadeUp .5s ease both;
  }
  ...
  .stat-tile{
    flex:1; min-width:100px; background:var(--bg); border:1px solid var(--line); border-radius:2px;
  ...
  .conv-bar-track{ height:4px; background:var(--bg-alt); border-radius:2px; overflow:hidden; margin-top:6px; }
  .conv-bar-fill{ height:100%; background:var(--pink); border-radius:2px; transition:width .8s cubic-bezier(.2,.8,.2,1); }
  ...
  td{ padding:14px 16px; border-bottom:1px solid var(--line); font-size:0.92rem; }
  ...
  .trial-banner{
    ...
    background:var(--canary); border:1px solid var(--line); box-shadow:var(--sh-sm);
    font-size:0.9rem; color:#111; font-weight:600; animation:fadeUp .5s ease both;
  }
  ...
  .trial-banner--urgente{ background:var(--coral); color:#111; }
  .trial-banner--vencido{ background:var(--coral); color:#111; }
  .trial-banner a{
  ...
  .code-box{ background:var(--bg-alt); border:1px solid var(--line); border-radius:2px; padding:12px 14px; }
  ...
  .btn-copy{ display:block; width:100%; background:var(--pink); color:#0d0f10; border:none; padding:8px 16px; border-radius:999px; font-size:0.82rem; font-weight:700; cursor:pointer; font-family:'Space Grotesk', sans-serif; box-shadow:var(--sh-sm); transition:transform .18s ease; }
  ...
  .app-card{
    ...
    border-radius:2px; padding:16px 18px; text-decoration:none; color:var(--ink);
    transition:transform .18s ease, border-color .18s ease; animation:fadeUp .5s ease both;
  }
  .app-card:hover{ transform:translateY(-2px); border-color:color-mix(in srgb, var(--pink) 45%, var(--line)); }
  .app-card:hover .app-icon{ transform:scale(1.15) rotate(-4deg); }
  .app-icon{ font-size:1.5rem; line-height:1; flex:none; margin-top:2px; display:inline-block; transition:transform .2s ease; }
  ...
  .app-badge{
    ...
    border:1px solid var(--line); font-weight:700; background:var(--mint); color:#0d0f10;
  }
  ...
  .settings-card{ background:var(--bg-card); border:1px solid var(--line); box-shadow:var(--sh-sm); border-radius:2px; padding:18px 22px; margin-bottom:24px; display:flex; flex-direction:column; align-items:flex-start; gap:14px; animation:fadeUp .5s ease both; }
  .umbral-label{ display:flex; flex-direction:column; gap:6px; font-size:0.85rem; color:var(--ink-dim); font-weight:600; }
  .umbral-label input{ width:100px; background:var(--bg-alt); border:1px solid var(--line); border-radius:2px; padding:8px 10px; font-size:0.9rem; font-family:'Space Grotesk', sans-serif; font-weight:600; }
  ...
  button{ width:100%; margin-top:14px; background:var(--pink); color:#0d0f10; border:none; padding:10px 20px; border-radius:999px; font-weight:700; cursor:pointer; box-shadow:var(--sh-sm); transition:transform .18s ease; font-family:'Space Grotesk', sans-serif; font-size:0.88rem; }
  ...
  <div class="wrap">
    ...
    <div class="status-hero">
      ...
      <div class="status-hero-stats">
        ...
        <div class="stat-tile"><span class="stat-num">${ctrAviso}%</span><span class="stat-label">Conversión</span><div class="conv-bar-track"><div class="conv-bar-fill" style="width:${Math.min(100, ctrAviso)}%"></div></div></div>
      </div>
    </div>
